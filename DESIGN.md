...
DB名は `coffee-cellar`、現行バージョンは `6`（v2でメニュー、v3で区切り表記、v4で紐付け・順序、v5で開封日、v6で備考を追加）。このDB名は同一オリジンで一つの本アプリを運用する前提で固定する。別テスト環境は別オリジンか別DB名を使い、本番データに接続しない。
...
| 編集 | 名前・焙煎度・焙煎日・備考を更新（在庫・アーカイブ共通）。ID・登録日時・状態・飲み終わり日時・開封日を保持 |
...
### JSON形式 v1（現行はv4。各豆に`presetId`、`openedDate`、`notes`、各プリセットに`order`を加える。v1〜v3も復元可能で、備考は空文字列に移行する）
...
列順は `name,roast,roastDate,openedDate,ageDays,status,finishedAt,notes`。`openedDate`は未開封が空欄、日付不明が`unknown`。在庫とアーカイブを両方含め、焙煎日・登録日時・IDの順で並べる。焙煎度は数字なら `2 / 5`、自由入力ならその文字列を使う。`ageDays`は書き出した現地日付まで、`finishedAt`はUTC日時または空欄。
