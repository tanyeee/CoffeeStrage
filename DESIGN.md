...
## 変更仕様：一覧上部の省スペース化（2026-09-22）

- 在庫・アーカイブの検索は英字見出しの右に置き、プレースホルダーを「検索」とする。通常幅は5.5rem、フォーカス中または検索語がある間は最大14remへ展開する。動きを減らす設定では幅のアニメーションを行わない。
- 在庫は「現在の貯蔵数 N袋」と「すべて / 未開封 / 開封済」を同じ段に置く。袋数は見出しの直後に表示する。
