...
## 変更仕様：在庫上部の操作を使用頻度に合わせる（2026-09-25）

- 在庫一覧には検索欄を表示しない。アーカイブの検索欄は従来どおり英字見出しの右に置き、プレースホルダーを「検索」とする。通常幅は5.5rem、フォーカス中または検索語がある間は最大14remへ展開する。動きを減らす設定では幅のアニメーションを行わない。
- 在庫ヘッダー右側には絞り込み前の袋数を「現在の貯蔵数 N袋」と表示する。在庫以外の画面では隠す。
- 在庫本文にはスクリーンリーダー向けの「在庫」見出しを残し、見える英字見出しは置かない。開封状態はボタン、期間と並び順は選択メニューにする。
- スマホでは **「並び順 / 期間 / すべて / 開封 / 未開封」** を右寄せの固定ドックにまとめ、豆追加ボタンと下部ナビの間へ置く。ドックの背景だけを透過させ、選択メニューの面は不透明な白にする。タップ面は44px以上とし、狭い幅ではドック内だけ横スクロールを許して開封ボタン側を最初に見せる。本文末尾に余白を確保する。
- デスクトップでは同じ順序の操作列を一覧内に表示する。
- アーカイブは豆フィルターと表示順を同じ段に置き、豆フィルターを左、表示順を右にする。期間と終了区分は次の段に置く。
...
- ヘッダーの袋数は在庫全体。絞り込み中は「該当 3 / 12袋 · 1ヶ月以上」と対象数も表示する。
- 在庫の検索欄は置かない。アーカイブ検索は維持する。
...
詳細画面の操作は上段に「編集」「開封」、下段に「アーカイブへ移す」を置く。375px幅で3つ横並びにすると文字が折り返すため段を分ける。開封ボタンはダイアログを開き、日付入力（初期値は今日、`min`は焙煎日、`max`は今日）、「保存」「日付不明」「未開封に戻す」「キャンセル」を出す。未開封の豆では「未開封に戻す」を隠す。アーカイブ済みの豆には開封ボタンを出さない。
...
詳細は豆名、焙煎度、焙煎日、現在までの経過年月・日数、登録日時を表示する。在庫には「編集」「アーカイブへ移す」を表示し、完全削除は置かない。

アーカイブへ移す際は「飲み切った」「人に譲渡」「廃棄した」から終了区分を選ぶ。保存後は同じ豆のアーカイブ詳細へ切り替え、選択した区分とアーカイブ日時を表示する。終了区分は編集画面から修正できる。在庫へ戻す機能は今後の候補とする。

アーカイブの初期表示は豆別で、プリセットの手動順に並べ、同じ豆の袋はアーカイブ日時の降順にする。各行には焙煎日からアーカイブ日までの保管期間、焙煎度、焙煎日、アーカイブ日を表示する。個別表示は終了日の新しい順・古い順を選べる。豆名、直近3ヶ月・1年以内、終了区分で絞り込み、豆名と備考を検索できる。詳細にはアーカイブ日時、終了区分、編集、完全削除を表示する。完全削除は「『豆名』を完全に削除しますか？ この操作は取り消せません。」と確認し、初期フォーカスをキャンセルに置く。

アーカイブ詳細の大きな経過表示は「焙煎から現在まで」と明記する。豆別一覧の「保管」は焙煎日からアーカイブ日までの固定期間とする。
...
DB名は `coffee-cellar`、現行バージョンは `7`（v2でメニュー、v3で区切り表記、v4で紐付け・順序、v5で開封日、v6で備考、v7で終了区分を追加）。このDB名は同一オリジンで一つの本アプリを運用する前提で固定する。別テスト環境は別オリジンか別DB名を使い、本番データに接続しない。
...
| `presetId` | 紐付くプリセットID。紐付けなしはnull。 |
| `notes` | 備考の文字列。未記入は空文字列。改行・前後空白を保持。一覧には表示せず、詳細で表示し追加・編集フォームで入力する。 |
...
| `finishedReason` | activeならnull。archivedは`consumed`、`gifted`、`discarded`、旧記録の未記録はnull。 |
| `openedDate` | 未開封はnull、日付不明は`unknown`、それ以外は実在する`YYYY-MM-DD`。焙煎日以降かつ今日以前。 |

プリセットは `id`、`name`、`order` を持つ。豆は `presetId` と名前を保持し、プリセットの名前変更時に紐付いた豆の名前も更新する。表示用の日数・年月は保存しない。
...
| 編集 | 名前・焙煎度・焙煎日・備考を更新（在庫・アーカイブ共通）。アーカイブでは終了区分も更新。ID・登録日時・状態・飲み終わり日時・開封日を保持 |
| 開封 | 詳細画面の専用ダイアログで`openedDate`だけを更新。追加・編集フォームでは扱わない |
| アーカイブへ移す | 終了区分を選び、activeをarchivedにして飲み終わり日時を設定 |
...
### JSON形式 v1（現行はv5。各豆に`presetId`、`openedDate`、`notes`、`finishedReason`、各プリセットに`order`を加える。v1〜v4も復元可能で、欠けている項目を安全な初期値へ移行する）
...
列順は `name,roast,roastDate,openedDate,ageDays,status,finishedAt,finishedReason,notes`。`openedDate`は未開封が空欄、日付不明が`unknown`。在庫とアーカイブを両方含め、焙煎日・登録日時・IDの順で並べる。焙煎度は数字なら `2 / 5`、自由入力ならその文字列を使う。`ageDays`は書き出した現地日付まで、`finishedAt`はUTC日時または空欄。
