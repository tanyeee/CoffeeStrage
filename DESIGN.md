...
- 在庫本文はスクリーンリーダー向けの「在庫」見出しと小さな `IN YOUR CELLAR` を置き、操作を **「すべて / 開封 / 未開封 / 期間 / 並び順」** の順に配置する。開封状態はボタン、期間と並び順は選択メニューにする。
- 2026-09-25追記：スマホでは5つの操作を透過背景の固定ドックにまとめ、豆追加ボタンと下部ナビの間へ置く。タップ面は44px以上とし、狭い幅ではドック内だけ横スクロールを許す。本文末尾にドックと豆追加ボタン分の余白を確保する。デスクトップでは一覧内の操作列を維持する。
