...
test('opening a bag records a date, 不明 and a revert to 未開封',async({page})=>{
 await page.clock.install({time:new Date('2026-09-17T12:00:00+09:00')});await seed(page);
 await expect(page.locator('.batch-opened').first()).toHaveText('未開封');
 await page.locator('.batch-row').first().click();
 await expect(page.getByRole('button',{name:'開封',exact:true})).toBeVisible();
 await page.getByRole('button',{name:'開封',exact:true}).click();
 await expect(page.locator('#opened-clear')).toBeHidden();
 await expect(page.locator('#opened-date')).toHaveValue('2026-09-17');
 await expect(page.locator('#opened-date')).toHaveAttribute('min','2025-02-14');
 await page.locator('#opened-date').fill('2025-02-13');await page.getByRole('button',{name:'保存'}).click();
 await expect(page.locator('#opened')).toBeVisible();
 expect(await page.locator('#opened-date').evaluate(input=>input.checkValidity())).toBe(false);
 await page.locator('#opened-date').fill('2025-06-01');await page.getByRole('button',{name:'保存'}).click();
 await expect(page.locator('.detail-grid')).toContainText('2025/06/01');
 await page.getByRole('link',{name:'在庫に戻る'}).click();
 await expect(page.locator('.batch-opened').first()).toHaveText('25/6/1');
 await page.locator('.batch-row').first().click();
 await page.getByRole('button',{name:'開封日を変更'}).click();
 await expect(page.locator('#opened-clear')).toBeVisible();
 await page.getByRole('button',{name:'日付不明'}).click();
